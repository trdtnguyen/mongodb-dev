    ...
    WT_FH *fh, wt_off_t start_off, wt_off_t len)
{
	...
	fprintf(my_fp_log,"%llu %f write file %s page_type log_zero offset %jd len %d \n",
			my_count, time_ms, fh->name, (off_t)off, wrlen );	
	my_count++;
#endif
	}
    ...
    WT_MYSLOT *myslot, bool force, WT_ITEM *record, WT_LSN *lsnp)
{
	...
	double time_ms = (my_tv.tv_sec) * 1000 + (my_tv.tv_usec) / 1000 ;
	time_ms = time_ms - my_start_time;
	fprintf(my_fp_log,"%llu %f write file %s page_type log_fill offset %jd start_offset %jd myslot->offset %jd len %zu \n",
			my_count, time_ms, myslot->slot->slot_fh->name,
		   	(myslot->offset + myslot->slot->slot_start_offset), myslot->slot->slot_start_offset,
		   	myslot->offset, (size_t)logrec->len );	
	my_count++;
#endif
...
__log_newfile(WT_SESSION_IMPL *session, bool conn_open, bool *created)
{
	...
	double time_ms = (my_tv.tv_sec) * 1000 + (my_tv.tv_usec) / 1000 ;
	time_ms = time_ms - my_start_time;
	fprintf(my_fp_log,"%llu %f new_log file %s fildeid %d is_create_log %d\n",
			my_count, time_ms, log->log_fh->name,log->fileid, create_log);
	my_count++;
#endif
	/*
...
__wt_log_release(WT_SESSION_IMPL *session, WT_LOGSLOT *slot, bool *freep)
{
	...
	if (release_buffered != 0){
		...
		double time_ms = (my_tv.tv_sec) * 1000 + (my_tv.tv_usec) / 1000 ;
		time_ms = time_ms - my_start_time;
		fprintf(my_fp_log,"%llu %f write file %s page_type wt_log_release offset %jd len %zu \n",
				my_count, time_ms, slot->slot_fh->name,
