...
#ifdef TDN_TRIM
/* default value for number written per fstrim call*/
//#define FSTRIM_FREQ 262144 
	extern FILE* my_fp4;
...
__wt_evict(WT_SESSION_IMPL *session, WT_REF *ref, bool closing)
{
...
#ifdef TDN_TRIM
	{
		...
		num_dirty_w = WT_STAT_READ(S2C(session)->stats, cache_eviction_dirty);	
		//if (my_count > FSTRIM_FREQ){
		//if ((num_dirty_w / FSTRIM_FREQ) > my_count){
		if ((int32_t)(num_dirty_w / my_trim_freq_config) > my_count){
			my_count = (num_dirty_w / my_trim_freq_config);	
		...
		//++my_count; //increase number of dirty page write
		//fprintf(my_fp4, "Check num_dirty_w %"PRId64" \n", num_dirty_w);
		//printf("Check my_count %"PRId64" \n", num_dirty_w);
	}
#else
		WT_ERR(__evict_page_dirty_update(session, ref, closing));
#endif
...
#ifdef TDN_TRACK_PID 
	fprintf(my_fp3, "__evict_page_clean_update closing= %d \n", closing);
#endif
	/*
...
__evict_page_dirty_update(WT_SESSION_IMPL *session, WT_REF *ref, bool closing)
{
	...
	case WT_PM_REC_REPLACE: 			/* 1-for-1 page swap */
		...
		*addr = mod->mod_replace;
		...
		__wt_ref_out(session, ref);
#ifdef TDN_TRACK_PID
		/*tdnguyen: increase eviction count for this ref */
		++ref->evict_count;
		//fprintf(my_fp3, "===evict_page_dirty_update: key %"PRIu64" page_type %u evict_count %u\n", 
		//		ref->key.recno, ref->page->dsk->type, ref->evict_count);
	fprintf(my_fp3, "__evict_page_dirty_update rec_result %d key %"PRIu64" evict_count %u \n", 
			mod->rec_result, ref->key.recno, ref->evict_count);
#endif
		ref->addr = addr;
		WT_PUBLISH(ref->state, WT_REF_DISK);
		break;
