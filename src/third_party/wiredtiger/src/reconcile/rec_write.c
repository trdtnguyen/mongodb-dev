    ...
    WT_REF *ref, WT_SALVAGE_COOKIE *salvage, uint32_t flags)
{
	...
	uint64_t oldest_id;
#ifdef TDN_TRACK_PID 
	const uint8_t *my_addr1, *my_addr2;
	size_t my_addr_size1, my_addr_size2;
	...
	my_addr1 = my_addr2 = NULL;

	//get info of the ref, root page is exception
	if (!(__wt_ref_is_root(ref))){		
		__wt_ref_info(ref, &my_addr1, &my_addr_size1, NULL);
		if(my_addr1 != NULL)
			WT_RET(__wt_block_buffer_to_addr(block, my_addr1, &my_offset1, &my_size1, &my_cksum1));
		else{
			//addr== NULL mean page is created in-memory
			my_cksum1 = 0; //only for debuging 
		}
	}
	...
	if (ret == 0){
		if (!(__wt_ref_is_root(ref))){		
			__wt_ref_info(r->ref, &my_addr2, &my_addr_size2, NULL);
			if(my_addr1 != NULL && my_addr2 != NULL){
				WT_RET(__wt_block_buffer_to_addr(block, my_addr2, &my_offset2, &my_size2, &my_cksum2));
				fprintf(my_fp3, "offset1 %jd size1 %u offset2 %jd size2 %u ", my_offset1, my_size1, my_offset2, my_size2);
			}
			else {
				//addr== NULL mean page is created in-memory
				fprintf(my_fp3, "page_created_in_memory ");
				my_cksum1 = 0; //only for debug
			}
		}
		else {
				fprintf(my_fp3, "root_page ");
		}

		fprintf(my_fp3, "before %d ", page->modify->rec_result);
		ret = __rec_write_wrapup(session, r, page);
		fprintf(my_fp3, "after %d \n", page->modify->rec_result);
	}
#else
	if (ret == 0)
		ret = __rec_write_wrapup(session, r, page);
...
//tdnguyen
	...
	double time_ms = (my_tv.tv_sec) * 1000 + (my_tv.tv_usec) / 1000 ;
	time_ms = time_ms - my_start_time;
	fprintf(my_fp,"in __wt_rec_root_write ... \n");
	my_count++;
#endif
...
/*tdnguyen modify*/
	...
	WT_ERR(__wt_bt_write(session,
	    buf, addr, &addr_size, false, bnd->already_compressed, page->type));
#elif SSDM_OP3
	WT_ERR(__wt_bt_write(session,
	    buf, addr, &addr_size, false, bnd->already_compressed, page->type, page->read_gen));
#else
	WT_ERR(__wt_bt_write(session,
...
__rec_write_wrapup(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_PAGE *page)
{
	 ...
	 */
#if defined(TDN_TRIM5)
	//fprintf(my_fp4, "__rec_write_wrapup mod->rec_result = %d\n", mod->rec_result);
#endif
	switch (mod->rec_result) {
	case 0:	/*
...
#elif TDN_TRIM5
		//similar with TDN_TRIM4 but use the obj->max_size as the threshold 
		if (!__wt_ref_is_root(ref)){
			/* Get the old address*/
	...
	case 0:						/* Page delete */
		...
		if (__wt_ref_is_root(ref))
			WT_RET(bm->checkpoint(
			    bm, session, NULL, btree->ckpt, false, page->type));
#elif SSDM_OP3
		if (__wt_ref_is_root(ref))
			WT_RET(bm->checkpoint(
			    bm, session, NULL, btree->ckpt, false, page->type));
...
/*tdnguyen modified*/
		...
		if (bnd->addr.addr == NULL)
			WT_RET(__wt_bt_write(session, &r->disk_image,
			    NULL, NULL, true, bnd->already_compressed, page->type));
#elif SSDM_OP3
		if (bnd->addr.addr == NULL)
			WT_RET(__wt_bt_write(session, &r->disk_image,
			    NULL, NULL, true, bnd->already_compressed, page->type, page->read_gen));
#else
		if (bnd->addr.addr == NULL)
			WT_RET(__wt_bt_write(session, &r->disk_image,
    ...
    WT_RECONCILE *r, WT_KV *kv, uint8_t type, uint64_t rle)
{
	...
	if (addr == NULL) {
		...
		/*tdnguyen modified */
#ifdef TDN
		WT_ERR(__wt_bt_write(session, tmp, addr, &size, false, false, page->type));
#elif SSDM_OP3
		WT_ERR(__wt_bt_write(session, tmp, addr, &size, false, false, page->type, page->read_gen));
#else
		WT_ERR(__wt_bt_write(session, tmp, addr, &size, false, false));
#endif
		/*end tdnguyen modified */
