...
__wt_bt_write(WT_SESSION_IMPL *session, WT_ITEM *buf,
    uint8_t *addr, size_t *addr_sizep, bool checkpoint, bool compressed, int page_type)
#elif SSDM_OP3
int
__wt_bt_write(WT_SESSION_IMPL *session, WT_ITEM *buf,
    uint8_t *addr, size_t *addr_sizep, bool checkpoint, bool compressed, int page_type, uint64_t page_read_gen)
#else
int
__wt_bt_write(WT_SESSION_IMPL *session, WT_ITEM *buf,
...
#ifdef TDN
	WT_ERR(checkpoint ?
	    bm->checkpoint(bm, session, ip, btree->ckpt, data_cksum, page_type) :
	    bm->write(bm, session, ip, addr, addr_sizep, data_cksum, page_type));
#elif SSDM_OP3
	WT_ERR(checkpoint ?
	    bm->checkpoint(bm, session, ip, btree->ckpt, data_cksum, page_type) :
	    bm->write(bm, session, ip, addr, addr_sizep, data_cksum, page_type, page_read_gen));
#else
	WT_ERR(checkpoint ?
