...
static int
 __bm_checkpoint(WT_BM *bm,
    WT_SESSION_IMPL *session, WT_ITEM *buf, WT_CKPT *ckptbase, bool data_cksum, int page_type)
{
	return (__wt_block_checkpoint(
	    session, bm->block, buf, ckptbase, data_cksum, page_type));
}
#elif SSDM_OP3
static int
 __bm_checkpoint(WT_BM *bm,
    ...
    WT_ITEM *buf, uint8_t *addr, size_t *addr_sizep, bool data_cksum, int page_type)
{
	return (__wt_block_write(
	    session, bm->block, buf, addr, addr_sizep, data_cksum, page_type));
}
...
{
	return (__wt_block_write(
	    session, bm->block, buf, addr, addr_sizep, data_cksum, page_type, page_read_gen));
}
#else
static int
__bm_write(WT_BM *bm, WT_SESSION_IMPL *session,
...
__bm_method_set(WT_BM *bm, bool readonly)
{
	if (readonly) {
		...
		bm->checkpoint = (int (*)(WT_BM *, WT_SESSION_IMPL *,
		    WT_ITEM *, WT_CKPT *, bool, int))__bm_readonly;
#elif SSDM_OP3
		bm->checkpoint = (int (*)(WT_BM *, WT_SESSION_IMPL *,
		    WT_ITEM *, WT_CKPT *, bool, int))__bm_readonly;
#else
		...
		bm->write = (int (*)(WT_BM *, WT_SESSION_IMPL *,
		    WT_ITEM *, uint8_t *, size_t *, bool, int))__bm_readonly;
#elif SSDM_OP3
		bm->write = (int (*)(WT_BM *, WT_SESSION_IMPL *,
		    WT_ITEM *, uint8_t *, size_t *, bool, int, uint64_t))__bm_readonly;
#else
		bm->write = (int (*)(WT_BM *, WT_SESSION_IMPL *,
