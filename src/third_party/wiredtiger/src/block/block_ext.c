...
int
__wt_block_alloc(
    WT_SESSION_IMPL *session, WT_BLOCK *block, wt_off_t *offp, wt_off_t size, int page_type)
#elif SSDM_OP3
int
__wt_block_alloc(
    WT_SESSION_IMPL *session, WT_BLOCK *block, wt_off_t *offp, wt_off_t size, int page_type, uint64_t page_read_gen)
#else
int
__wt_block_alloc(
    WT_SESSION_IMPL *session, WT_BLOCK *block, wt_off_t *offp, wt_off_t size)
...
append:			WT_RET(__block_extend(session, block, offp, size));
			...
			fprintf(my_fp2,"%llu %f write file %s page_type %d offset %jd size %jd allocated_by append \n",
					my_count++, time_ms, block->fh->name, page_type, *offp, size);	
			return (0);
		}
...
append:			WT_RET(__block_extend(session, block, offp, size));
			...
			fprintf(my_fp2,"%llu %f write file %s page_type %d offset %jd size %jd allocated_by append read_gen %jd\n",
					my_count++, time_ms, block->fh->name, page_type, *offp, size, page_read_gen);	
			return (0);
		}
...
append:			WT_RET(__block_extend(session, block, offp, size));
	...
	*offp = ext->off;
	...
	double time_ms = (my_tv.tv_sec) * 1000 + (my_tv.tv_usec) / 1000 ;
	time_ms = time_ms - my_start_time;
	fprintf(my_fp2,"%llu %f write file %s page_type %d offset %jd size %jd allocated_by %s \n",
			my_count++, time_ms, block->fh->name, page_type, *offp, size, ((my_temp == 1) ? "first-fit" : "best-fit"));	
#elif SSDM_OP3
	gettimeofday(&my_tv, NULL);
	double time_ms = (my_tv.tv_sec) * 1000 + (my_tv.tv_usec) / 1000 ;
	time_ms = time_ms - my_start_time;
	fprintf(my_fp2,"%llu %f write file %s page_type %d offset %jd size %jd allocated_by %s read_gen %jd\n",
			my_count++, time_ms, block->fh->name, page_type, *offp, size, ((my_temp == 1) ? "first-fit" : "best-fit"), page_read_gen);	
#endif
	return (0);
}
...
#define	WT_EXTLIST_WRITE(p, v)						\
	...
	/*tdnguyen*/
#ifdef TDN
	//page_type = 100 mean extend list page
 	WT_ERR(__wt_block_write_off(session,
	    block, tmp, &el->offset, &el->size, &el->cksum, true, true, 9));

#elif SSDM_OP3
	//writes from checkpoints has page_read_gen =3 
	WT_ERR(__wt_block_write_off(session,
	    block, tmp, &el->offset, &el->size, &el->cksum, true, true, 9, 3));
#else
	WT_ERR(__wt_block_write_off(session,
