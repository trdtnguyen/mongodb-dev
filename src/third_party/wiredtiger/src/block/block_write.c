...
int
 __wt_block_write(WT_SESSION_IMPL *session, WT_BLOCK *block,
    WT_ITEM *buf, uint8_t *addr, size_t *addr_sizep, bool data_cksum, int page_type)
{
	wt_off_t offset;
	uint32_t size, cksum;
	uint8_t *endp;
	...
	WT_RET(__wt_block_write_off(
	    session, block, buf, &offset, &size, &cksum, data_cksum, false, page_type));
...
int
__wt_block_write(WT_SESSION_IMPL *session, WT_BLOCK *block,
    WT_ITEM *buf, uint8_t *addr, size_t *addr_sizep, bool data_cksum, int page_type, uint64_t page_read_gen)
{
	wt_off_t offset;
	uint32_t size, cksum;
	uint8_t *endp;
	...
	WT_RET(__wt_block_write_off(
	    session, block, buf, &offset, &size, &cksum, data_cksum, false, page_type, page_read_gen));
...
__wt_block_write_off(WT_SESSION_IMPL *session, WT_BLOCK *block,
    ...
    bool data_cksum, bool caller_locked, int page_type)
#elif SSDM_OP3
int
__wt_block_write_off(WT_SESSION_IMPL *session, WT_BLOCK *block,
    WT_ITEM *buf, wt_off_t *offsetp, uint32_t *sizep, uint32_t *cksump,
    bool data_cksum, bool caller_locked, int page_type, uint64_t page_read_gen)
#else
int
__wt_block_write_off(WT_SESSION_IMPL *session, WT_BLOCK *block,
    WT_ITEM *buf, wt_off_t *offsetp, uint32_t *sizep, uint32_t *cksump,
...
#ifdef TDN
	ret = __wt_block_alloc(session, block, &offset, (wt_off_t)align_size, page_type);
#elif SSDM_OP3
	ret = __wt_block_alloc(session, block, &offset, (wt_off_t)align_size, page_type, page_read_gen);
#else
...
/*
	...
	fprintf(my_fp,"%llu %f write file %s page_type %d offset %jd size %jd \n",
			my_count, time_ms, fh->name, page_type, offset, align_size);	
	my_count++;
#endif
*/
	/* Write the block. */
